...
const createBook = async function (req, res) {
  try {
    ...
    let { bookCover,title, excerpt, userId, ISBN, category, subcategory, releasedAt } =
    ...
    // }
    //  let findBookCover = await bookmodel.findOne({bookCover: bookCover});

    //  if(findBookCover)
    //  return res.status(400).send({status:false, message: "BookCover is already exist"})
    ...
    if (!checkids) {
      return res
        .status(404)
        .send({ status: false, message: "UserId is not found" });
    }
    ...
    if (typeof requestBody.subcategory === "undefined"  || requestBody.subcategory === "null" )
          return res.status(400).send({ status: false, msg:"please enter subcategory key or valid subcategory"})
      if (requestBody.subcategory.length == 0) {
          return res.status(400).send({ status: false, msg: "subcategory is not valid" });
      }
           
            if(subcategory){
                if((typeof requestBody.subcategory!=="string") || (typeof requestBody.subcategory==="object") ){
             for (let i = 0; i < requestBody.subcategory.length; i++) {
                if (subcategory[i] == 0) {
